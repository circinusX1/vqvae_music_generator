...
def sub_main(genere):
    ...
    vqvae_checkpoint = torch.load(vqvae_path, map_location=device, weights_only=True)
    if isinstance(vqvae_checkpoint, dict) and "model_state_dict" in vqvae_checkpoint:
        vqvae_checkpoint = vqvae_checkpoint["model_state_dict"]
    cleaned_vqvae_state = {
        k.replace("_orig_mod.", ""): v for k, v in vqvae_checkpoint.items()
    }
    vqvae.load_state_dict(cleaned_vqvae_state)
    vqvae.eval()
    ...
    if use_lora:
        optimizer = torch.optim.Adam(transformer.get_trainable_params(), lr=float(cfg['training']['lr']))
    else:
        optimizer = torch.optim.Adam(transformer.parameters(), lr=float(cfg['training']['lr']))
    ...
    if os.path.exists(checkpoint_path):
        ...
        # OVERRIDE LR: Force optimizer to use the learning rate from config.yaml
        for param_group in optimizer.param_groups:
            param_group['lr'] = float(cfg['training']['lr'])
        print(f"-> Overrode optimizer LR to config value: {cfg['training']['lr']}")
    ...
    good_gen_loss = float(cfg['training'].get('good_gen_loss', 0.0))
    ...
    for epoch in range(start_epoch, cfg['training']['gen_epochs']):
        ...
        if good_gen_loss > 0 and avg_loss <= good_gen_loss:
            print(f"\n🎯 Target loss reached: {avg_loss:.4f} <= {good_gen_loss:.4f}! Stopping training early.")
            break
